...
def renderFrame(width, height):
    ...
    beamModelToWorldTransform = baseModelToWorldTransform * lu.make_translation(0, 15.0, 3.0 + g_beamOffset)
    drawObjModel(viewToClipTfm, worldToViewTfm, beamModelToWorldTransform, g_beamModel)

    weightModelToWorldTransform = beamModelToWorldTransform * lu.make_translation(0, 0.0, -9.0)
    drawObjModel(viewToClipTfm, worldToViewTfm, weightModelToWorldTransform, g_weightModel)

    ballModelToWorldTransform = beamModelToWorldTransform * lu.make_translation(0, -13.0, 7.0)
    drawObjModel(viewToClipTfm, worldToViewTfm, ballModelToWorldTransform, g_ballModel)

    cableModelToWorldTransform = beamModelToWorldTransform * lu.make_translation(0, -6.0, 7.0)
    drawObjModel(viewToClipTfm, worldToViewTfm, cableModelToWorldTransform, g_cableModel)

    for i in range(0,8):
        extraBallModelToWorldTransform = ballModelToWorldTransform * lu.make_rotation_y(math.radians(i * 360/8) + time.time() * 1.0) * lu.make_translation(0, 0, 2.0) * lu.make_scale(0.3, 0.3, 0.3)
        drawObjModel(viewToClipTfm, worldToViewTfm, extraBallModelToWorldTransform, g_ballModel)
        for j in range(0,8):
            extraBallModelToWorldTransform2 = extraBallModelToWorldTransform * lu.make_rotation_x(math.radians(j * 360/8) + time.time() * 1.0) * lu.make_translation(0, 3.0, 0.0)
            drawObjModel(viewToClipTfm, worldToViewTfm, extraBallModelToWorldTransform2, g_ballModel)
